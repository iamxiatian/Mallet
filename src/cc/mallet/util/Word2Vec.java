...
public class Word2Vec {
    ...
    public List<String> getSimilarWords(String word, int topN, float minScore) {
        List<String> words = new ArrayList<>();
        Set<WordEntry> entires = distance(word);
        if (entires == null) {
            return words;
        }

        int count = 0;
        for (WordEntry entry : entires) {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        String formatString = "Word2Vec -f model_file [-d distance]|[-a " +
                "analogy]|[-v input vector for distance]";
        ...
        options.addOption(new Option("v", false, "test vector"));
        ...
        if (cmdLine.hasOption("a")) {
            ...
            while (true) {
                ...
                float[] values = vec.getWordVector(list);
                Set<WordEntry> entries = vec.distance(list);
                if (entries == null) {
            ...
            System.out.println("Start vector test (Type EXIT to exit)...");
            while (true) {
                System.out.print("Enter float vector:");
                String word = scanner.nextLine();
                if (word.equalsIgnoreCase("exit")) {
                    System.out.println("Bye.");
                    break;
                ...
                for (int i = 0; i < elements.length; i++) {
                    values[i] = Float.parseFloat(elements[i]);
                }
                Set<WordEntry> entries = vec.distance(values);
                System.out.println(Arrays.toString(values));
                for (WordEntry entry : entries) {
                    System.out.println(entry.name + "\t" + entry.score);
                }
