...
public class ACRF implements Serializable {
    ...
    public Inferencer getInferencer() {
        return globalInferencer;
    }

    public void setInferencer(Inferencer inf) {
        globalInferencer = inf;
    }
...
//				dumpAssnForClique (assn, clique);
    ...
    public void setVerboseOutputDirectory(File dir) {
        verboseOutputDirectory = dir;
    }
...
//					System.out.println ("ACRF "+this+" ["+i+"] has index "+indices[j]);
        ...
        private int allocateDefaultWeights(int size) {
            ...
            if (defaultWeights != null)
                newdefaultWeights.plusEqualsSparse(defaultWeights);
...
*/
        ...
        public SparseVector[] getExpectations(int cnum) {
            return expectations[cnum];
        }

        public SparseVector[] getConstraints(int cnum) {
            return constraints[cnum];
        }
      ...
      /* Fill in expectations for each instance */
        ...
        /* Compute marginals for each clique */
                ...
                if (unrolled.numVariables() == 0)
                    continue;   // Happens if all nodes are pruned.
