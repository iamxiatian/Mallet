...
public class CPT implements DiscreteFactor {
    ...
    public double value(Assignment assn) {
        return subFactor.value(assn);
    }

    public double value(AssignmentIterator it) {
        return subFactor.value(it);
    }

    public Factor normalize() {
        return subFactor.normalize();
    }

    public Factor marginalize(Variable[] vars) {
        return subFactor.marginalize(vars);
    }
    ...
    public Factor marginalize(Variable var) {
        return subFactor.marginalize(var);
    }

    public Factor marginalizeOut(Variable var) {
        return subFactor.marginalizeOut(var);
    }

    public Factor extractMax(Collection vars) {
        return subFactor.extractMax(vars);
    }

    public Factor extractMax(Variable var) {
        return subFactor.extractMax(var);
    }
    ...
    public int argmax() {
        return subFactor.argmax();
    }

    public Assignment sample(Randoms r) {
        return subFactor.sample(r);
    }

    public double sum() {
        return subFactor.sum();
    }

    public double entropy() {
        return subFactor.entropy();
    }

    public Factor multiply(Factor dist) {
        return subFactor.multiply(dist);
    }

    public void multiplyBy(Factor pot) {
        subFactor.multiplyBy(pot);
    }

    public void exponentiate(double power) {
        subFactor.exponentiate(power);
    }

    public void divideBy(Factor pot) {
        subFactor.divideBy(pot);
    }

    public boolean containsVar(Variable var) {
        return subFactor.containsVar(var);
    }

    public VarSet varSet() {
        return subFactor.varSet();
    }

    public AssignmentIterator assignmentIterator() {
        return subFactor.assignmentIterator();
    }

    public boolean almostEquals(Factor p) {
        return subFactor.almostEquals(p);
    }

    public boolean almostEquals(Factor p, double epsilon) {
        return subFactor.almostEquals(p, epsilon);
    }

    public Factor duplicate() {
        return subFactor.duplicate();
    }

    public boolean isNaN() {
        return subFactor.isNaN();
    }

    public double logValue(AssignmentIterator it) {
        return subFactor.logValue(it);
    }

    public double logValue(Assignment assn) {
        return subFactor.logValue(assn);
    }

    public double logValue(int loc) {
        return subFactor.logValue(loc);
    }

    public Variable getVariable(int i) {
        return subFactor.getVariable(i);
    }

    public int sampleLocation(Randoms r) {
        return subFactor.sampleLocation(r);
    }

    public double value(int index) {
        return subFactor.value(index);
    }

    public int numLocations() {
        return subFactor.numLocations();
    }

    public double valueAtLocation(int loc) {
        return subFactor.valueAtLocation(loc);
    }

    public int indexAtLocation(int loc) {
        return subFactor.indexAtLocation(loc);
    }

    public double[] toValueArray() {
        return subFactor.toValueArray();
    }

    public int singleIndex(int[] smallDims) {
        return subFactor.singleIndex(smallDims);
    }

    public String dumpToString() {
        return subFactor.dumpToString();
    }

    public Factor slice(Assignment assn) {
        return subFactor.slice(assn);
    }
