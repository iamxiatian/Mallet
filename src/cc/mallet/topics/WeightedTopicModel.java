        ...
        for (int position = 0; position < docLength; position++) {
            ...
            for (int topic = 0; topic < numTopics; topic++) {
                ...
                if (debugging && type == 68) {
                    System.out.println(type + "\t" + topic + "\t" + localTopicCounts[topic] + "\t" + currentTypeTopicCounts[topic] + "\t" + currentTypeTopicWeights[topic] + "\t" + tokensPerTopic[topic] + "\t" + sum);
                }
            }
    ...
    public void printState(PrintStream stream) {
        ...
        for (int doc = 0; doc < data.size(); doc++) {
            ...
            for (int position = 0; position < topicSequence.getLength(); position++) {
                ...
                out.append(type);
                out.append(' ');
                out.append(alphabet.lookupObject(type));
                out.append(' ');
                out.append(topic);
                out.append("\n");
            }
